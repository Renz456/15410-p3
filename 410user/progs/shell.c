...
/* Not all kernels support getchar().
 *
 * Try to use it, but if it doesn't work fall back to readline().
 */
...
{
  static int missing_getchar = 0;
  int c;

  if (missing_getchar || (c = getchar()) < 0) {
    missing_getchar = 1;
...
static void print_ls()
{
  ...
  while ((num_entries = ls(size, buf)) < 0) {
    size *= 2;
    char *nbuf = (char *)realloc(buf, sizeof(char) * size);
    if (nbuf == NULL) {
      printf("Can't realloc(%d)!\n", sizeof(char) * size);
      goto cleanup;
    } else {
      buf = nbuf;
    }
