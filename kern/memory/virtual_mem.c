...
void set_zero_page(pde *pd_start)
{
    add_frame(PAGE_SIZE, (unsigned int)ZFOD_ADDR_PA,
              pd_start, KERN_PD_FLAG, KERN_PT_FLAG);
    memset((void *)PAGE_SIZE, 0, PAGE_SIZE);
    remove_frame(PAGE_SIZE, pd_start);
}
