...
pic_acknowledge_any_slave(void)
{
    ...
    /** @brief Test if a given IRQ is active.
     *
     * @pre    Arguably interrupts should be off.
     *
     * @param  irq The IRQ, from the master PIC's perspective, that is to
     *         be acknowledged.  This means that 0-7 are master IRQs and
     *         8-15 are slave.
     *
     * @return True if active, false if not (including bogus IRQ values)
     */
int
pic_irq_active(unsigned char irq)
{
	unsigned char port;
	unsigned char mask;
    ...
    outb(port, READ_IS_ONRD | OCW_TEMPLATE | READ_NEXT_RD);
    return ((inb(port) & mask) != 0);
}
